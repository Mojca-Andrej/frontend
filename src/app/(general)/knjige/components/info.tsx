        ...
        <main>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-screen-xl">
                <div className="md:col-span-2">
                    ...
                    <h2 className="text-neutral-700 md:text-xl text-lg py-2">{book.type}</h2>
                    <section className="flex flex-col space-y-2 mb-8 text-neutral-900 md:text-lg">
                        ...
                        {book.ilustrator && <p><span className="font-semibold">Ilustrator: </span>{book.ilustrator}</p>}
                        {book.cd && <p><span className="font-semibold">CD uglasbenih pesmi: </span>{book.cd}</p>}
                        <p><span className="font-semibold">Avtor: </span>{book.author}</p>
                ...
                <div className="md:col-span-1 flex items-center justify-center">
                    <Image src={`/${book.image}`} alt={book.title} height={800} width={800} className="w-2/3 sm:w-1/2 md:w-full xl:w-2/3 aspect-A4 rounded-sm shadow-md" />
                </div>
            </div>
        </main>
    );
