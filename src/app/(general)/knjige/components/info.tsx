    ...
    return (
        <main className="bg-white px-4 md:px-16 xl:px-32 py-8">
            <div className="flex flex-wrap w-full max-w-screen-lg">
                <div className="md:w-1/3 py-4 xl:pr-8 pr-4 w-full justify-center items-center flex ">
                    <Image src={`/${book.image}`} alt={book.title} height={800} width={800} className="md:w-full w-3/4 md:max-h-[400px]" />
                </div>
                <div className="md:w-2/3 p-4">
                    <div className="">
                    <h1 className="text-black font-semibold text-2xl">{book.title}</h1>
                    <h2 className="text-neutral-500 text-xl py-2">{book.type}</h2>
                    <p className="text-black text-lg py-2" dangerouslySetInnerHTML={{ __html: book.description }}></p>
